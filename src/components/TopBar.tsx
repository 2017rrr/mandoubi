        ...
        <Sheet>
          <SheetTrigger asChild>
            <button className="relative p-2">
              ...
              {unreadCount > 0 && (
                <span className="absolute -top-0.5 -left-0.5 bg-destructive text-destructive-foreground text-[10px] w-4 h-4 rounded-full flex items-center justify-center font-bold">
                  {unreadCount}
                </span>
              )}
            </button>
              ...
              {notifications.length === 0 ? (
                ...
                notifications.map(n => (
                  <button
                    ...
                    className={`w-full text-right p-4 border-b border-border hover:bg-card transition-colors ${!n.is_read ? 'bg-primary/5' : ''}`}
                  >
